...
const chromeDriverPath = path.resolve(__dirname,'../../driver/chromedriver.exe');
    ...
    it('deberia poder buscar un matenimiento con exito', async function() {
        this.timeout(60000); // Aumenta el timeout para esta prueba
        ...
        try {
            ...
            console.log('Haciendo clic en "Área de mantenimientos"...');
            await waitAndInteract(By.linkText("Área de mantenimientos"), 'click');

            await waitAndInteract(By.css(".form-control:nth-child(1)"), 'click');
            await waitAndInteract(By.css(".form-control:nth-child(1)"), 'sendKeys', '679287');
    ...
    it('deberia poder agregar un Mantenimiento con exito', async function() {
        ...
        try {
            ...
            await waitAndInteract(By.css(".carousel-control-next-icon"), 'click');
            await waitAndInteract(By.css(".carousel-control-next-icon"), 'click');
            
            await waitAndInteract(By.css(".carousel-item:nth-child(3) .btn:nth-child(1)"),'click');

await waitAndInteract(By.name("issue_description"), 'click');
await waitAndInteract(By.name("issue_description"), 'sendKeys', ' 2');
await waitAndInteract(By.name("unit_mileage"), 'sendKeys', ' 2');
await waitAndInteract(By.name("requires_platform_transfer"), 'click');
await waitAndInteract(By.name("under_warranty"), 'click');
await waitAndInteract(By.name("mechanic_contact"), 'sendKeys', ' 2');

let campo1 = await driver.findElement(By.name('mechanic_phone'));
...
await waitAndInteract(By.name("mechanic_phone"), 'sendKeys', '99999999');
await waitAndInteract(By.name("observations"), 'sendKeys',' 2');
