...
const chromeDriverPath = path.resolve(__dirname,'../../driver/chromedriver.exe');
    ...
    it('deberia poder agregar un vehiculo con exito', async function() {
        this.timeout(60000); // Aumenta el timeout para esta prueba
...
await waitAndInteract(By.css(".d-flex > .btn"), 'click');
await waitAndInteract(By.name("region"), 'click');
await waitAndInteract(By.name("region"), 'sendKeys', 'Prueba1');
await waitAndInteract(By.name("dependency"), 'sendKeys', 'Prue');
await waitAndInteract(By.name("plate"), 'sendKeys', '111444');
await waitAndInteract(By.name("asset_code"), 'sendKeys','PRB');
await waitAndInteract(By.name("heritage"), 'sendKeys', 'PRB-1');
await waitAndInteract(By.name("brand"), 'sendKeys', 'Toyota');
await waitAndInteract(By.name("style"), 'sendKeys', 'suzuki');
await waitAndInteract(By.name("model_year"), 'sendKeys','2009');
